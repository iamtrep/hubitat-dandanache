...
mappings {
    ...
    path("/hub-metrics.json") {action: [GET: "getHubMetricsMapping"]}
}
...
def getHubMetricsMapping() {
    debug "Returning Hub metrics"
    ...
    location.hubs.each { hub ->
        hubs.add """\
        {
            "name": "${hub.name}",
            "ip": "${hub.localIP}",
            "uptime": ${hub.uptime.toLong()},
            "fw": "${hub.firmwareVersionString}"
        }
        """
    }
