        ...
        if (app.getInstallationState() == "COMPLETE") {
            ...
            section {
                ...
                href (
                    ...
                    url: "${getLocalURL(HTML_FILE_NAME)}",
                    style: "embedded",
                    state: "complete",
                    required: false,
                )
...
def getCloudURL(String fileName) {
    return "${getApiServerUrl()}/${fileName}?access_token=${state.accessToken}"
}
...
mappings {
    path("/${HTML_FILE_NAME}") {action: [GET: "loadMemCpuHistoryMapping"]}
    path("/app.webmanifest") {action: [GET: "loadAppManifestMapping"]}
}
...
def loadAppManifestMapping() {
    ...
    return render(
        ...
        data: """\
        {
            ...
            "start_url": "${getLocalURL(HTML_FILE_NAME)}",
            "icons": [{
            ...
            "theme_color": "${useDarkTheme ? "#002b36" : "#eee8d5"}",
            "background_color": "${useDarkTheme ? "#002b36" : "#eee8d5"}"
        }
        """
    )
}
